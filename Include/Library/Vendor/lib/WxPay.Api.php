...
class WxPayApi
{
	 ...
	 *
	 */
	public static function swift_pay_submit($inputObj, $timeOut = 6){
		$url = WxPayConfig::SWIFT_URL?WxPayConfig::SWIFT_URL:'https://pay.swiftpass.cn/pay/gateway';
		//检测必填参数
		...
		}else if(!$inputObj->IsServiceSet()) {
			throw new WxPayException("缺少统一支付接口必填参数service！");
		}
		...
		//$inputObj->SetAppid(WxPayConfig::XINGYEID);//公众账号ID
		$inputObj->SetMch_id(WxPayConfig::XINGYEID);//商户号
		$inputObj->SetMch_create_ip($_SERVER['REMOTE_ADDR']);//终端ip	    	    
